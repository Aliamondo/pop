...
//						if (resourceListCompareId(res.paletteId,bufferedPalette) /*TODO: add &&!paletteCheckCompatibility(currentPalette,image) */) { /* The palette isn't in the buffer */
							...
							if ((err=mReadFileInDatFileId(&otherPalette))==PR_RESULT_SUCCESS) {
								...
								printf("type=%d\n",currentPalette.type);
								e.id=/*res*/currentPalette.id;
								list_insert(&paletteBuffer,(void*)&e);
								printf("loaded necessary palette\n");
							} /* else, that's bad, I'll have to use the previous palette, even if it is the default */
							else{printf("can't load necessary palette, error=%d\n",err);}
#endif
/*printf("palette is not in buffer IDX=%s ID=%d ORD=%d\n",res.paletteId.index,res.paletteId.value,res.paletteId.order);*/
currentPalette.type=eResTypeNone;
						} /* else, good, the palette is buffered */
						else{currentPalette=((tPaletteListItem*)list_getCursor(&paletteBuffer))->pal; /*printf("good, the palette is buffered HAS=%d NEED=%d\n",((tPaletteListItem*)list_getCursor(&paletteBuffer))->id.value,res.paletteId.value);*/}
